...
if __name__ == "__main__":
  ...
  if gpu_device is not None:
      ...
      if gpu_id_locked < 0:
          # automatic lock removal has time delay of 2 so be sure to have the lock of the last run removed we wait for 3s here
          time.sleep(3)
          # try a second time
          gpu_id_locked=gpl.obtain_lock_id(id=gpu_device)
      ...
      # obtain_lock_id positions CUDA_VISIBLE_DEVICES such that only the selected GPU is visibale, therefore we need now select /GPU:0
      comp_device = "/GPU:0"
  else:
      # computer has no gpu, so use cpu instead
      comp_device = "/cpu:0" 
  
  print('comp_device = {}'.format(comp_device))
  # runs main()
  tf.app.run()
